...
const DEFAULTS = {
  ...
  maxInfraStreak: 3,        // consecutive never-reached-the-model failures before the breaker trips
};
...
function plan(ctx = {}) {
  ...
  if (ctx.busy) return { run: false, reason: 'agent is busy — foreground work owns the machine', retry: true };
  if (ctx.lockHeld) return { run: false, reason: 'another background worker holds the lock', retry: true };
  if (!ctx.idleSince || now - ctx.idleSince < cfg.minIdleMs) {
    // `retry` marks a refusal that RESOLVES ON ITS OWN, shortly. The boot tick lands at 90s against
    // a 120s idle threshold, so it is a guaranteed no — and without this hint the worker then waits
    // a full 20-minute cycle for a condition that cleared 30 seconds later. Same shape as the lock
    // skip: a transient refusal must not cost a whole period. Caps and "nothing to do" are NOT
    // transient and deliberately do not set it.
    return { run: false, reason: `idle for less than ${Math.round(cfg.minIdleMs / 1000)}s`, retry: true };
  }
  const st = ctx.state || {};
  ...
  // CIRCUIT BREAKER. If the last few runs never reached the model, the next one will not either —
  // the fault is in the machine, not the queue. Marching on just converts one broken thing into a
  // night of identical failures against every item in the backlog, which is exactly what happened.
  // Deliberately NOT `retry`: this must not clear itself on a short timer, because nothing about
  // waiting 30 seconds fixes it. It clears on a restart or on the next run that genuinely happens.
  if ((st.infraStreak || 0) >= (cfg.maxInfraStreak || DEFAULTS.maxInfraStreak)) {
    return { run: false, broken: true,
      reason: `${st.infraStreak} runs in a row never reached the model — pausing rather than failing every item in turn` +
        (st.last && st.last.error ? ` (last: ${String(st.last.error).slice(0, 120)})` : '') };
  }
...
async function runItem(item, deps = {}, opts = {}) {
  ...
  try {
    ...
    return { ok, id, agent: item.agent, item, ms: now() - t0, usd: (res && res.usd) || 0, text: (res && (res.result || res.text)) || '',
      ...(ok ? {} : { error: (res && res.error) || 'the specialist reported failure without a reason' }) };
  } catch (e) {
...
/**
 * Did this run FAIL WITHOUT EVER REACHING THE MODEL?
 *
 * The distinction matters more than it looks. A genuine attempt — the specialist read the code,
 * thought, and came up short — is real evidence about the ITEM, and earns the 3-day cooldown. A
 * failure that never got past the wire says nothing about the item at all; it says something about
 * the machine, and it will repeat identically on the next item.
 *
 * Live, that difference cost a whole night: a malformed-history 400 failed six items in under a
 * second each, and every one was marked attempted. The queue emptied, the worker announced "every
 * open item was attempted recently", and went quiet for three days having done nothing.
 *
 * A run that reached the model always bills something, so zero spend AND zero output is the signal.
 */
function neverRan(result) {
  if (!result || result.ok) return false;
  return !(result.usd > 0) && !String(result.text || '').trim();
}
...
function record(state, item, result, now) {
  ...
  const infra = neverRan(result);
  return {
    day: today,
    // An attempt that never reached the model does not count against the daily item cap either —
    // the cap exists to bound work and spend, and this was neither.
    countToday: (sameDay ? (state.countToday || 0) : 0) + (infra ? 0 : 1),
    spentUsd: +(((sameDay ? (state.spentUsd || 0) : 0) + ((result && result.usd) || 0)).toFixed(4)),
    // Consecutive never-reached-the-model failures. Reset by any run that actually happened, so a
    // one-off blip costs nothing and a real outage trips the breaker in plan().
    infraStreak: infra ? ((state.infraStreak || 0) + 1) : 0,
    // The item is only marked attempted when the attempt was real.
    attempted: infra ? { ...(state.attempted || {}) } : { ...(state.attempted || {}), [itemKey(item)]: now },
    last: {
...
module.exports = { plan, parseBacklog, routeFor, itemKey, briefFor, runItem, record, neverRan, DEFAULTS, ROUTES };
