...
// ---- fact similarity (SoT dedup) — Notion is the authoritative store for durable facts, so we
// avoid writing near-duplicates and de-duplicate on recall so divergent copies never pile up. ----
function normFact(s) { return String(s || '').toLowerCase().replace(/[^a-z0-9 ]+/g, ' ').replace(/\s+/g, ' ').trim(); }
function factTokens(s) { return new Set(normFact(s).split(' ').filter((w) => w.length > 2)); }
function similar(a, b) {
  const na = normFact(a), nb = normFact(b);
  if (!na || !nb) return false;
  if (na === nb || na.includes(nb) || nb.includes(na)) return true;
  const ta = factTokens(a), tb = factTokens(b);
  if (!ta.size || !tb.size) return false;
  let inter = 0; for (const t of ta) if (tb.has(t)) inter++;
  return inter / (ta.size + tb.size - inter) >= 0.8;   // Jaccard
}
function dedupFacts(arr) {
  const kept = [];
  for (const item of arr) { const f = item && (item.fact != null) ? item.fact : item; if (!kept.some((k) => similar(k.fact != null ? k.fact : k, f))) kept.push(item); }
  return kept;
}

/** Newest durable facts from the Memory db (authoritative ordering by created_time). */
async function recentMemory({ limit = 50 } = {}) {
  if (!isConfigured() || !cfg().memoryDbId) return [];
  const nc = client(); if (!nc) return [];
  try {
...
async function appendMemory({ fact, tags = [], source = 'agent', confidence = 0.8, dedup = true } = {}) {
  ...
  try {
    // SoT dedup: skip writing a near-duplicate of a recent fact (prevents silent divergence).
    if (dedup) {
      const dup = (await recentMemory({ limit: 60 })).find((m) => similar(m.fact, fact));
      if (dup) return { success: true, deduped: true, reason: 'duplicate of an existing fact' };
    }
...
async function searchMemory(query, { limit = 5 } = {}) {
  ...
  try {
    ...
    return dedupFacts((r.results || []).map((p) => ({
      fact: propText(findProp(p.properties, ['Fact', 'Name'])),
      tags: propText(findProp(p.properties, ['Tags'])),
      date: propText(findProp(p.properties, ['Date'])),
    })));
...
/**
 * Best-effort recall: title-contains first, then a token-overlap pass over recent facts as a
 * fallback (so "what did I say about the FABLE bug" surfaces even without an exact title match),
 * de-duplicated. Returns [] (never throws) when Notion is unconfigured.
 */
async function recall(query, limit = 6) {
  if (!isConfigured() || !cfg().memoryDbId) return [];
  const direct = await searchMemory(query, { limit });
  let facts = Array.isArray(direct) ? direct.slice() : [];
  if (facts.length < limit) {
    const ql = factTokens(query);
    const scored = (await recentMemory({ limit: 100 }))
      .map((m) => { const tt = factTokens(m.fact); let i = 0; for (const t of ql) if (tt.has(t)) i++; return { m, s: i }; })
      .filter((x) => x.s > 0).sort((a, b) => b.s - a.s).map((x) => x.m);
    facts = facts.concat(scored);
  }
  return dedupFacts(facts).slice(0, limit);
}
...
module.exports = { isConfigured, appendMemory, searchMemory, recall, recentMemory, logActivity, createResearchPage, updateProjectState, getOpenTasks };
