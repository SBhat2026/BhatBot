...
function load() {
  let raw;
  try { raw = fs.readFileSync(CREDS_FILE, 'utf8'); }
  catch { return {}; }                                  // genuinely absent: nothing vaulted yet
  try { return JSON.parse(raw); }
  catch (e) {
    // Present but unparseable. Returning {} here would read as "no credentials stored", which is
    // how a damaged vault becomes an app that silently loses every integration at once.
    try { console.error(`[vault] ⚠ ${CREDS_FILE} exists but does not parse (${e.message}) — credentials are UNREADABLE, not absent`); } catch {}
    return {};
  }
}
// ATOMIC. This file IS the vault — a truncated write loses every credential at once, and there is
// no backup and no way to re-derive them. Write to a temp file and rename() (atomic on POSIX), so a
// reader or a crash sees the old file or the new one, never half of one.
function save(c) {
