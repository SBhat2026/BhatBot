...
function store(label, domain, username, secret) {
  ...
  const ref = `CRED_REF_${String(label).toUpperCase().replace(/\W+/g, '_')}_${Date.now().toString(36).toUpperCase()}`;
  const c = load();
...
module.exports = { store, resolve, list, remove, resolveRefs, hasRef, CREDS_FILE };
