...
// Sweep aged-out run directories. Every deploy_drones call mints a ~/.bhatbot/drones/run-<ts>/ holding
// a blackboard + a run record, and nothing ever removed them — a long-lived install accumulates them
// indefinitely. Only prunes directories matching `prefix`, and only once they're past maxAgeDays, so
// an in-flight run is never touched. Pure-ish: returns what it removed.
function pruneRunDirs(dir, { prefix = 'run-', maxAgeDays = 14, now = Date.now() } = {}) {
  const removed = [];
  let entries = [];
  try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { return { removed, scanned: 0 }; }
...
// Run ONE maintenance pass. deps = { semanticMaintain?(), wsCompress?(), trimLogs?:[paths],
// pruneDirs?:[{dir,maxAgeDays,prefix}], onReport?(r) }.
// Every step is optional + guarded so a missing/failed store never breaks the pass.
async function runPass(deps = {}) {
  const report = { at: deps.now || null, semantic: null, workspaces: null, logs: [], dirs: [] };
  ...
  for (const d of deps.pruneDirs || []) {
    try { report.dirs.push({ dir: d.dir, ...pruneRunDirs(d.dir, d) }); } catch (e) { report.dirs.push({ dir: d.dir, error: e.message }); }
  }
...
module.exports = { planMaintenance, trimLog, pruneRunDirs, runPass, start, stop, status };
