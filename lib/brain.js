...
function createBrain({ dir } = {}) {
  ...
  function graphView({ includePruned = false } = {}) {
    ...
    return { nodes: ns, edges: es, stats: stats() };
  }

  return { upsertNode, upsertEdge, getNode, nodes, edges, neighbors, prune, confirm, stats, graphView, save, load, dir: base, _store: () => store };
}
