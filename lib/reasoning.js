...
const CAPS = {
  'claude-opus-5':     { thinking: 'adaptive',  effort: ['low', 'medium', 'high', 'xhigh', 'max'], compact: true,  taskBudget: true,  midSystem: true,  structured: true },
  'claude-sonnet-5':   { thinking: 'adaptive',  effort: ['low', 'medium', 'high', 'xhigh', 'max'], compact: true,  taskBudget: false, midSystem: false, structured: true },
  'claude-opus-4-8':   { thinking: 'adaptive',  effort: ['low', 'medium', 'high', 'xhigh', 'max'], compact: true,  taskBudget: true,  midSystem: true,  structured: true },
  'claude-opus-4-7':   { thinking: 'adaptive',  effort: ['low', 'medium', 'high', 'xhigh', 'max'], compact: true,  taskBudget: true,  midSystem: true,  structured: true },
  'claude-opus-4-6':   { thinking: 'adaptive',  effort: ['low', 'medium', 'high', 'max'],          compact: true,  taskBudget: false, midSystem: false, structured: true },
  'claude-fable-5':    { thinking: 'always-on', effort: ['low', 'medium', 'high', 'xhigh', 'max'], compact: true,  taskBudget: true,  midSystem: false, structured: true },
  'claude-sonnet-4-6': { thinking: 'adaptive',  effort: ['low', 'medium', 'high', 'max'],          compact: true,  taskBudget: false, midSystem: false, structured: true },
  'claude-haiku-4-5':  { thinking: null,        effort: null,                                     compact: false, taskBudget: false, midSystem: false, structured: false },
};
...
// `make_model` is here for the same reason: its result is a set of paths to artifacts that still
// exist on disk, so clearing it leaves "open that lamp again" with nothing to open.
const EXCLUDE_FROM_CLEARING = ['save_memory', 'ask_options', 'ask_form', 'self_reflect', 'request_permissions', 'make_model'];
...
// Depth tier → tool-result clearing trigger, inside Anthropic's published 30K–100K band.
// Cheap turns keep almost no tool history; deep work keeps enough to reason over.
const DEPTH_CLEAR_TRIGGER = { ack: 30000, conversational: 30000, detailed: 50000, deep: 80000 };
...
function shapeRequest(body, opts = {}) {
  ...
  // ---- structured output ---------------------------------------------------
  // Several paths ask the model for JSON and then dig it out of prose with a regex
  // (`txt.match(/\[[\s\S]*\]/)`). When the model gets chatty, wraps the JSON in a fence, or
  // answers with a preamble, the match fails and the caller degrades to "found nothing" — which
  // reads exactly like a legitimately empty result. `output_config.format` makes the API enforce the
  // schema instead, so a malformed answer becomes impossible rather than invisible.
  // Verified supported on Opus 5 and Sonnet 5 via GET /v1/models (`structured_outputs.supported`).
  // Shape is exactly { type, schema }. A `name` field is NOT accepted — the API returns
  // 400 "output_config.format.name: Extra inputs are not permitted". Callers may still pass
  // schemaName for logging; it just never goes on the wire.
  const fmt = opts.schema && caps.structured ? { type: 'json_schema', schema: opts.schema } : null;

  if (effort || tb || fmt) {
    out.output_config = { ...(out.output_config || {}) };
    if (effort) out.output_config.effort = effort;
    if (tb) out.output_config.task_budget = tb;
    if (fmt) out.output_config.format = fmt;
  }
  ...
  if (caps.compact && !out.context_management) {
    ...
    //
    // THE NUMBERS ARE SOURCED, NOT GUESSED. Anthropic's context-engineering cookbook runs clearing
    // at trigger 30K / keep 4 / clear_at_least 10K, and gives 30K–100K, 3–6 and 10K as the working
    // ranges. My first pass used 40K / 3 / 5K from reasoning alone; clear_at_least was HALF the
    // recommended floor, which matters because too small a clear invalidates the cache prefix
    // without freeing enough to pay for the re-write.
    //
    // The trigger is scaled by DEPTH inside that published band rather than fixed, which is the
    // point: an 'ack' turn has no use for 100K of tool history, and a 'deep' coding turn is
    // crippled without it. BhatBot already classifies every turn (lib/depth.js) and that
    // classification drives max_tokens and effort — this is the same question asked about HISTORY,
    // so it reuses the same answer instead of inventing a second policy that can drift.
    //
    // Chroma's context-rot study (18 frontier models, 2025) is the reason the band tops out where
    // it does: measurable accuracy loss appears well before the window is full — a 200K-window
    // model can degrade noticeably by ~50K of input — so "we have 1M tokens" is not a reason to
    // keep 1M tokens.
    if (opts.clearToolUses !== false) {
      edits.push({
        type: 'clear_tool_uses_20250919',
        trigger: { type: 'input_tokens', value: opts.clearTrigger || DEPTH_CLEAR_TRIGGER[opts.depth] || 50000 },
...
module.exports = {
  ...
  CAPS, DEPTH_EFFORT, DEPTH_CLEAR_TRIGGER, TASK_BUDGET_MIN, BETA_COMPACT, BETA_CONTEXT, BETA_TASK_BUDGET, EXCLUDE_FROM_CLEARING,
};
