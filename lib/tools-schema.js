...
module.exports = function toolSchema({ MEMORY_SECTIONS = [] } = {}) {
  ...
  { name: 'ask_options', description: 'Ask Siddhant to CHOOSE from a set of options with an INTERACTIVE CHECKBOX CARD shown right in the app — instead of listing choices as text he has to read and type back. Use this ANY time you would otherwise write "pick one / choose any / which of these / do you want A, B, or C" (loadout features, colour schemes, plan variants, scope choices, confirmations with extras). He taps his choice(s) and you receive them back. PREFER this over a text list whenever the options are discrete. multi:true → checkboxes + a Confirm button (he can pick several); multi:false/omit → single-select (one tap submits). Returns { selected: [chosen labels] }. Desktop app only; on phone/headless it returns an error so you fall back to a text list.',
    input_schema: { type: 'object', properties: {
      question: { type: 'string', description: 'The prompt shown above the options, e.g. "Pick your suit loadout".' },
      options: { type: 'array', description: '2–12 choices. Each is { label, description? } — label is the short choice, description is an optional one-liner explaining it.',
