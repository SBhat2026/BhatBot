...
module.exports = function toolSchema({ MEMORY_SECTIONS = [] } = {}) {
  ...
  { name: 'self_drive', description: 'AUTONOMOUS PROACTIVE SELF-IMPROVEMENT loop (Phase 6 — the always-on engine that ACTS on self_reflect\'s desires). On a timer while idle it: reflects on what to improve, picks the top desire, runs a multi-agent pipeline (SCOUT researches → ORACLE/ECHO plan → FORGE/ATLAS code+verify), and KEEPS the change only if `npm run verify` passes (else reverts) — committing+pushing on an isolated `self-drive` branch. ENABLED by default (aggressive). Hard rails: verify-or-revert, frozen guardrail/secret zone, idle-only, daily cap, cooldown, rate-budget-paced. action:"status" shows state; "enable"/"disable" toggle; "run"/"cycle" forces one improvement cycle now. Use "status" for "are you improving yourself / what\'s self-drive doing", "run" for "go improve yourself now", "disable" to stop the autonomous loop.',
    input_schema: { type: 'object', properties: {
      action: { type: 'string', enum: ['status', 'enable', 'disable', 'run', 'cycle'] }
    }, required: ['action'] } },
