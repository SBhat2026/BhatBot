...
module.exports = function toolSchema({ MEMORY_SECTIONS = [] } = {}) {
  ...
  { name: 'file_tools', description: "FILE & IMAGE UTILITY BELT — convert, resize, compress and rotate images; merge/split/extract/rotate PDFs and pull their text; convert audio/video; render PDF pages to images. Runs LOCALLY on Siddhant's Mac (sips / ffmpeg / pypdf / PIL) so files never leave the machine and it works offline. Actions: `capabilities` (what this Mac can currently do — check first if unsure), `run` (perform `op` on `path`, or `paths` for merge). Ops: image_convert, image_resize, image_compress, image_rotate, image_info, pdf_merge, pdf_split, pdf_extract, pdf_rotate, pdf_text, pdf_info, pdf_to_images, images_to_pdf, media_convert, media_info, audio_extract, and (only if installed) remove_background, ocr, vectorize. If an op is unavailable the result names the missing dependency, how to install it locally, and a thetoolbus.ai page to do it by hand. Prefer this over writing a script for these jobs.",
    input_schema: { type: 'object', properties: {
      action: { type: 'string', enum: ['capabilities', 'run'], description: 'capabilities = list what this Mac can do; run = perform an operation.' },
      op: { type: 'string', description: 'run: the operation, e.g. image_resize / pdf_merge / pdf_text.' },
      path: { type: 'string', description: 'run: absolute path to the input file.' },
      paths: { type: 'array', items: { type: 'string' }, description: 'run: input files for pdf_merge / images_to_pdf (order matters).' },
      out: { type: 'string', description: 'run: output path or directory (optional — a sensible name is derived otherwise).' },
      format: { type: 'string', description: 'image_convert (png/jpeg/tiff/heic), media_convert (mp4/mov/webm), audio_extract (mp3/wav/m4a).' },
      maxDimension: { type: 'number', description: 'image_resize: longest side in pixels.' },
      quality: { type: 'number', description: 'image_compress: 0-100 (mapped to sips low/normal/high/best).' },
      degrees: { type: 'number', description: 'image_rotate / pdf_rotate: 90, 180 or 270.' },
      from: { type: 'number', description: 'pdf_extract: first page (1-based, inclusive).' },
      to: { type: 'number', description: 'pdf_extract: last page (inclusive).' }
    }, required: ['action'] } },
  { name: 'triage_mail', description: "EMAIL TRIAGE LEDGER — inspect and REVERSE what BhatBot did to Siddhant's inbox on its own. Actions: `recent` (the last N ledger entries: what was classified, which rule fired, what action was taken), `stats` (per-class counts + top senders — what the review week reads), `undo` (reverse ONE action by its ledger `id`, restoring the message's exact prior labels), `undo_all` (bulk rollback, optionally `since` an ISO timestamp — e.g. everything archived this morning), `run` (force one triage pass now instead of waiting for the timer), `pin` (add a sender to NEVER_ARCHIVE so it is never auto-archived again). Use `undo_all` when Siddhant says something like 'undo what you archived today' or 'put those back'.",
    input_schema: { type: 'object', properties: {
      action: { type: 'string', enum: ['recent', 'stats', 'undo', 'undo_all', 'run', 'pin'], description: 'What to do.' },
      id: { type: 'string', description: 'undo: the ledger entry id (act_000123) from `recent`.' },
      since: { type: 'string', description: 'undo_all/stats/recent: ISO timestamp or date — only entries at or after this.' },
      limit: { type: 'number', description: 'recent: how many entries (default 30).' },
      sender: { type: 'string', description: 'pin: the email address to never auto-archive again.' }
    }, required: ['action'] } },
  ...
  { name: 'visual_build', description: "Build a VIDEO from a description (ByteDance Seedance, via the hosted MCP connector). Use for motion: product/feature demos, animated logos and title cards, b-roll, storyboard shots, an idea shown moving rather than described. This is the video counterpart to generate_image — reach for generate_image when a still will do, since video costs far more and takes ~1-2 minutes. Generation is asynchronous but THIS TOOL WAITS: one call returns the finished video, already downloaded, with a poster frame returned to you as a vision block so you can judge the result and re-run with a corrected prompt. Pass `image` (a local path or URL) to animate a still instead of generating from scratch — that is how you turn a generate_image output, a screenshot, or a rendered 3D frame into motion. If it reports it is not authenticated, use the `connectors` tool.",
    input_schema: { type: 'object', properties: {
      prompt: { type: 'string', description: 'The shot to build. Be cinematic and specific: subject, action, camera move, lighting, style, pacing.' },
      image: { type: 'string', description: 'Optional first-frame reference (URL or local path) — animates this still instead of generating from nothing.' },
      images: { type: 'array', items: { type: 'string' }, description: 'Optional [first_frame, last_frame] pair to control start and end.' },
      model: { type: 'string', description: 'Optional Seedance model id. Omit for the default; call mcp__seedance__seedance_list_models to see options and pricing.' },
      resolution: { type: 'string', enum: ['480p', '720p', '1080p', '4k'], description: 'Default 720p. 1080p/4k cost materially more.' },
      aspect_ratio: { type: 'string', enum: ['16:9', '9:16', '1:1', '4:3', '3:4', '21:9', 'adaptive'], description: '9:16 for phone/social, 16:9 for demos.' },
      duration: { type: 'number', description: 'Seconds of video.' },
      audio: { type: 'boolean', description: 'Generate synchronized audio (1.5 Pro / 2.0 series only).' },
      open: { type: 'boolean', description: 'Open the finished video in the default player. Default true.' }
    }, required: ['prompt'] } },
  { name: 'connectors', description: "Inspect or enable BhatBot's hosted MCP connectors — the same remote integrations Claude.ai connects to (currently: seedance). action:'list' (default) reports each connector's url, whether it is enabled, whether a token is present, and which of its tools are live right now. action:'enable' with `name` turns one on and reconnects the hub immediately. Pass `token` ONLY if Siddhant supplies one in the conversation — never invent one, and prefer telling him to set the environment variable so it is vaulted rather than typed. Use this when a connector-backed tool reports it is not authenticated.",
    input_schema: { type: 'object', properties: {
      action: { type: 'string', enum: ['list', 'enable'], description: 'Default list.' },
      name: { type: 'string', description: 'Connector name, e.g. "seedance".' },
      token: { type: 'string', description: 'API token. Stored encrypted in the vault, never written as plaintext.' }
    } } },
  ...
  { name: 'math_reason', description: 'Solve a COMPLEX, multi-step MATH / quantitative-reasoning problem with a code-first agent (smolagents) that writes and EXECUTES Python (numpy/sympy/scipy authorized) to compute a verifiable answer — not a guessed one. Use for hard algebra/calculus/number-theory/probability/optimization word problems, derivations, or anything where step-by-step computation beats mental math. Returns the final answer plus the code it ran. Runs in the simulation sandbox; needs scripts/sim-setup.sh.',
    input_schema: { type: 'object', properties: {
      ...
      model: { type: 'string', description: 'Reasoning model (default claude-sonnet-5). Use a stronger model for harder problems.' },
  ...
  { name: 'mission_plan', description: 'YOUR DURABLE PLAN for a long task. Write the plan down here instead of holding it in your head: this file survives context trimming, a rate-limit pause, and even the app restarting, and it is read back to you automatically every time your context gets summarized. action:"set"{items} lays out the steps — do this ONCE, early, for any task with more than ~3 stages. action:"mark"{index,status,note?} updates one step as you go: mark it "doing" when you start and "done" the moment it is genuinely finished, not when you intend to finish it. action:"read" shows the current plan. Marking steps done is what stops you redoing work after a long run — an unmarked plan is worse than no plan. Only meaningful inside a mission (a long, action-shaped task); a no-op on a short turn.',
    input_schema: { type: 'object', properties: {
      action: { type: 'string', enum: ['set', 'mark', 'read'], description: 'set = lay out the whole plan; mark = update one step; read = show it.' },
      items: { type: 'array', description: 'For set: the ordered steps as short imperative strings ("read lib/fleet.js", "add the bounded pool", "run npm run verify").', items: { type: 'string' } },
      index: { type: 'number', description: 'For mark: which step (0-based, as shown by read).' },
      status: { type: 'string', enum: ['todo', 'doing', 'done', 'blocked'], description: 'For mark: the new status of that step.' },
      note: { type: 'string', description: 'For mark: a short why — say what is blocking when marking "blocked".' }
    }, required: ['action'] } },
  ...
  { name: 'make_asset', description: "BUILD AN ASSET END TO END — text → image → 3D model → printable STL, in ONE call. This is the tool to reach for whenever the goal is an OBJECT rather than a picture. `want`: 'image' (a picture — same as generate_image), 'model' (a textured GLB you can spin and drop into a scene), 'printable' (an STL ready for a 3D printer). It runs whichever stages that target needs and returns every artifact it produced, so you do NOT chain generate_image → generate_3d → make_printable yourself; doing that costs a full conversation replay per stage. Pass `image` instead of `prompt` to build from a picture that already exists (a screenshot, a photo, an earlier generation). IT ALSO WRITES A BETTER SOURCE IMAGE THAN YOU WOULD: single-view 3D reconstruction needs a centred subject on a plain background with flat lighting and no shadow, and it appends exactly that when the target is 3D — so describe the OBJECT and let it handle the framing. If a stage fails you still get the artifacts from the stages that succeeded.",
    input_schema: { type: 'object', properties: {
      prompt: { type: 'string', description: 'What the object IS. Describe the thing, not the photograph — framing and lighting are added for you when the target is 3D.' },
      image: { type: 'string', description: 'Absolute path to an existing image to build from, instead of generating one.' },
      want: { type: 'string', enum: ['image', 'model', 'printable'], description: "'image' = picture only. 'model' = textured GLB (needs replicateKey). 'printable' = STL for a 3D printer. Default 'image'." },
      quality: { type: 'string', enum: ['low', 'medium', 'high'], description: 'Image quality. Defaults to high when the target is 3D, since the mesh is only as good as its source.' },
      size: { type: 'string', enum: ['1024x1024', '1536x1024', '1024x1536'], description: 'Default 1024x1024 — square reconstructs best.' },
      texture_size: { type: 'number', enum: [512, 1024, 2048], description: 'GLB texture resolution. Default 1024.' },
      mode: { type: 'string', enum: ['extrude', 'relief'], description: 'Only when going straight from an image to STL with no mesh step: extrude (silhouette solid — logos, keychains) or relief (lithophane/height-map).' },
      height_mm: { type: 'number', description: 'Print height in mm.' },
      width_mm: { type: 'number', description: 'Print width in mm.' },
      filename: { type: 'string', description: 'Base filename for the artifacts (no extension).' },
      open: { type: 'boolean', description: 'Open the finished model in the 3D viewer. Default true.' }
    } } },
  { name: 'make_model', description: "BUILD A 3D MODEL IN BLENDER — designed geometry, animation and export in ONE call, run headless. Use this when the object should be CONSTRUCTED (parametric shapes, named parts, modifiers, a rig, a turntable or any keyframed motion, exact dimensions); use `make_asset` instead when it should be RECONSTRUCTED from a picture. You write the bpy Python in `script`; everything around it is handled — an empty scene, metric units, lights, a camera automatically framed on whatever you actually built, a render after every step, triangle counts, and GLB/STL/OBJ export. A window opens showing your script, the live renders and the finished model, so the user watches it happen. SPLIT THE SCRIPT WITH `#--step: <name>` COMMENT MARKERS: each chunk is timed, rendered and reported on its own, so a failure names the step instead of a line number, and you get a picture of the geometry at each stage. Steps share one namespace, so a variable from one step is available in the next. AVAILABLE: `bpy` and `math` are pre-imported; you may import bpy, bmesh, mathutils, math, random, colorsys, itertools, functools, json and NOTHING else — no os, no file IO, no open(); return objects in the scene and they get exported for you. Do NOT add a camera, lights, a world, or an export call; do NOT save files. For animation just keyframe and call `_frames(n)` to set the range. `_paint(obj, (r,g,b))` gives an object a colour. If a step fails you still get the artifacts and previews from the steps that succeeded, plus the traceback.",
    input_schema: { type: 'object', properties: {
      script: { type: 'string', description: 'The bpy Python to run. Split it with `#--step: name` marker comments. No imports beyond the allow-list, no file IO, no camera/light/export/save — those are provided.' },
      ops: { type: 'array', description: 'Declarative alternative to `script` for simple builds — no Python needed. Each entry: {op:"cube|sphere|cylinder|cone|torus|plane|monkey", name?, size?, location?:[x,y,z], scale?, rotation?, color?:[r,g,b]}, or {op:"modifier", target?, type:"SUBSURF|BEVEL|…", settings?}, or {op:"spin", target?, frames?}. Can be combined with `script`, which runs after.', items: { type: 'object' } },
      name: { type: 'string', description: 'Base name for the artifacts and the build folder. Lower-case, no extension.' },
      formats: { type: 'array', items: { type: 'string', enum: ['glb', 'stl', 'obj'] }, description: "What to export. Default ['glb']. Add 'stl' when it is destined for a 3D printer." },
      preview_width: { type: 'number', description: 'Render width for the step previews. Default 512.' },
      preview_height: { type: 'number', description: 'Render height. Default 384.' },
      previews: { type: 'boolean', description: 'Render a preview after every step. Default true — turn it off only for a big batch where the pictures do not matter.' },
      show: { type: 'boolean', description: 'Open the Blender Studio window. Default true.' },
      timeout_ms: { type: 'number', description: 'Hard limit on the whole build. Default 240000 (4 min), max 600000.' }
    } } },
