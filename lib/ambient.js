...
const DEFAULTS = {
  ...
  // Volume cap, the other half of "don't be annoying". quietHours controls WHEN we may interrupt;
  // this controls HOW OFTEN. Without it a chatty morning of mail could surface a dozen signals
  // before 09:00 and train you to ignore the channel — and it becomes load-bearing the moment
  // background agents run unattended, because then nobody is watching the volume but this number.
  // `high` urgency is exempt: a budget that silences something genuinely urgent is worse than none.
  maxPerDay: 6,
  lookaheadMin: 120,     // calendar window to look ahead
  mailLookbackHours: 24, // mail window to look back (overnight); on-demand read can override (e.g. 168 = past week)
...
async function watchMail(cfg) {
  ...
  // Mail's unified `inbox` is a property (all accounts), not a class — `every inbox` is a syntax
  // error. Read it directly; fall back to each account's mailboxes named "INBOX" if that fails.
  const script = `
set outText to ""
  ...
  try
    ...
    repeat with m in msgs
      try
        ...
        set outText to outText & (subject of m) & "${FS_FIELD}" & (sender of m) & "${FS_FIELD}" & acctName & "${RS_REC}"
      end try
    end repeat
  on error
    repeat with acct in accounts
      try
        repeat with mb in (mailboxes of acct whose name is "INBOX")
          set msgs to (messages of mb whose read status is false and date received ≥ cutoff)
          repeat with m in msgs
            try
              set outText to outText & (subject of m) & "${FS_FIELD}" & (sender of m) & "${FS_FIELD}" & (name of acct) & "${RS_REC}"
            end try
          end repeat
        end repeat
      end try
...
return outText`;
  ...
  for (const rec of r.out.split(RS_REC)) {
    if (!rec.trim()) continue;
    const [subject, sender, account] = rec.split(FS_FIELD);
    candidates.push({ subject: (subject || '(no subject)').trim(), sender: (sender || '').trim(), account: (account || '').trim() });
  }
  ...
  // ACCOUNT COVERAGE. Mail's unified `inbox` already spans every account, but nothing said so — a
  // brief could report "no important email" having silently read one mailbox. Surface which accounts
  // were actually seen so a missing one is obvious rather than invisible.
  const accounts = [...new Set(candidates.map((c) => c.account).filter(Boolean))];
  const coverage = { accounts, accountCount: accounts.length, scanned: candidates.length };

  if (needsReply.length === 0) return { signals: [], coverage };
...
function saveSeen(map) {
  try {
    ...
    // Atomic. The daily notification budget is DERIVED from this file (surfacedToday counts entries
    // stamped today), so a truncated write does not merely lose dedup state — it silently resets
    // how many times BhatBot thinks it has interrupted you.
...
/**
 * How many signals were surfaced since local midnight.
 *
 * Derived from the seen store rather than kept as its own counter, because markSurfaced() is the
 * ONLY writer to it and it is only ever called with signals that were actually surfaced — so the
 * store already IS the record of what interrupted you today. A separate counter would be a second
 * source of truth that could drift from the first, which is the bug this codebase keeps producing.
 * SEEN_CAP (500) is far above any sane daily budget, so pruning cannot corrupt the count.
 */
function surfacedToday(seen, now = Date.now()) {
  const midnight = new Date(now); midnight.setHours(0, 0, 0, 0);
  const start = midnight.getTime();
  let n = 0;
  for (const ts of Object.values(seen || {})) if (typeof ts === 'number' && ts >= start) n++;
  return n;
}
...
async function scan() {
  ...
  let surfacing = quiet ? [] : out;
  let held = 0;
  const used = surfacedToday(seen, now);
  const max = Number.isFinite(cfg.maxPerDay) ? cfg.maxPerDay : DEFAULTS.maxPerDay;

  // DAILY BUDGET. Same contract as quiet hours: anything held back is NOT marked seen, so it
  // surfaces on a later tick rather than being lost. `high` urgency ignores the budget entirely —
  // the cap exists to stop noise training you to ignore the channel, not to suppress a real alarm.
  if (!quiet && max > 0) {
    const room = Math.max(0, max - used);
    if (surfacing.length > room) {
      const urgent = surfacing.filter((s) => s.urgency === 'high');
      const rest = surfacing.filter((s) => s.urgency !== 'high').slice(0, room);
      held = surfacing.length - urgent.length - rest.length;
      surfacing = [...urgent, ...rest];
    }
  }

  const result = { signals: surfacing, ts: new Date(now).toISOString() };
  if (quiet) result.quiet = true;
  // Always report the budget, even when nothing was held: a cap you cannot see is indistinguishable
  // from a watcher that has quietly stopped finding anything.
  result.budget = { used, max, held };
...
// scanSource(name) — run ONE watcher on demand, bypassing the master switch, the per-source
// enabled gate, quiet hours, and the seen/dedup store. For pull use (the morning brief reads mail
// without needing always-on ambient mail notifications). Returns redacted signals + a digest.
async function scanSource(name, opts = {}) {
  const fn = WATCHERS[name];
  if (!fn) return { error: 'unknown source: ' + name, signals: [], count: 0, result: 'Unknown source: ' + name };
  const base = loadConfig();
  const hours = Math.max(1, Math.min(720, Number(opts.hours) || (name === 'mail' ? base.mailLookbackHours || DEFAULTS.mailLookbackHours : 0)));
  const cfg = { ...base, mailLookbackHours: hours };   // honor a requested window (e.g. "past week" → 168)
  let res;
  try { res = await fn(cfg); }
  catch (e) { return { error: e && e.message ? e.message : 'watcher threw', signals: [], count: 0, result: 'Could not read ' + name + '.' }; }
  if (res && res.error) return { source: name, signals: [], count: 0, error: res.error, result: 'Could not read ' + name + ': ' + res.error };
  // watcher signal text is ALREADY redacted — don't redact again (it would eat sender names that
  // look like domains, e.g. "Chess.com" → "").
  const signals = ((res && res.signals) || []).map((s) => ({ source: name, kind: s.kind, text: s.text, urgency: s.urgency || 'low' }));
  // EXPLICIT empty result so the model states "nothing notable" instead of inventing items.
  const result = signals.length ? digest(signals)
    : `NOTHING TO REPORT: no ${name} items needing attention in the last ${hours}h. Tell the user there's nothing notable — do NOT invent any ${name === 'mail' ? 'emails' : 'items'}.`;
  return { source: name, signals, count: signals.length, digest: digest(signals), result, windowHours: hours };
}
