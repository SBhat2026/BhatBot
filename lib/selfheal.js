...
const DEFAULTS = {
  ...
  // The verify gate a fix MUST pass (exit 0) or it auto-reverts. Full suite, not just parse:
  // `npm run verify` = verify-syntax (parse + export contracts) + the functional unit/security
  // tests, so a fix that parses but breaks BEHAVIOR is caught + reverted. Fails CLOSED (if the
  // command can't run, the fix is treated as unverified → reverted). Override per config.selfHeal.verify.
...
function enqueue(m, loadConfig) {
  ...
  QUEUE.push({ key, problem: m.problem, verify: m.verify || cfg.verify, files: m.files || '', source: m.source || 'manual', at: Date.now() });
...
// Phase 6 mutual-exclusion: selfheal (reactive) YIELDS to selfdrive (proactive) — they'd otherwise
// compete for OTPM budget and could conflict on file edits. selfdrive writes a fresh lockfile while a
// session runs; selfHealTick checks this and skips. Stale locks (>2h, e.g. a crash) are ignored.
const SELFDRIVE_LOCK = path.join(os.homedir(), '.bhatbot', '.selfdrive.lock');
function selfdriveActive() {
  try {
    const st = fs.statSync(SELFDRIVE_LOCK);
    return (Date.now() - st.mtimeMs) < 2 * 60 * 60 * 1000;   // fresh lock = a session is live
  } catch { return false; }
}
// Today's self-heal fix count — so selfdrive can enforce a COMBINED (selfheal + selfdrive) daily cap.
function dayCount() { const s = loadState(); return s.day === today() ? (s.countToday || 0) : 0; }
...
module.exports = { DEFAULTS, cfgFrom, enabled, enqueue, pending, clusterAudit, gate, runOne, tick, status, selfdriveActive, dayCount, STATE_PATH };
