...
// A hard PER-DAY dollar cap (default $1/day) gates both paid passes, so an always-on worker cannot
// run away. The ledger is persistent, in graph meta — a restart must not reset the day's spend. It is
// deliberately a RATE: the original cumulative version spent its $1 once and then disabled the second
// brain permanently, silently, for ten days.
...
function createSynapse({ dir, deps = {} } = {}) {
  ...
  // ── cost governor ───────────────────────────────────────────────────────────────────────────────
  // ⚠ LOAD-BEARING — the budget is a RATE, not a lifetime allowance. Do not make `spendUsd`
  // cumulative again.
  //
  // It was: one ledger, incremented forever, checked against a $1 cap. So the second brain got $1 of
  // thinking in total, for its entire existence — and having spent it (on 22 August, at
  // $1.0003/$1.00) it could never embed, explain or suggest again. Nothing said so; the worker went
  // on ticking, reporting itself enabled, doing nothing. A cap that can only ever be hit once is not
  // a budget, it is a fuse.
  //
  // Now it is per-DAY, with the lifetime figure kept separately because it is genuinely interesting
  // — it just must not be the thing that gates spending.
  const budget = () => { const v = Number(loadConfig().synapseBudgetUsd); return Number.isFinite(v) && v > 0 ? v : 1; };
  const today = () => new Date(now()).toISOString().slice(0, 10);
  function spent() {
    try { return b.getMeta('spendDay') === today() ? (Number(b.getMeta('spendTodayUsd')) || 0) : 0; }
    catch { return 0; }
  }
  const spentLifetime = () => { try { return Number(b.getMeta('spendUsd')) || 0; } catch { return 0; } };
  const left = () => Math.max(0, budget() - spent());
  const charge = (usd) => {
    try {
      const amt = Number(usd) || 0;
      b.setMeta('spendDay', today());
      b.setMeta('spendTodayUsd', spent() + amt);
      b.setMeta('spendUsd', spentLifetime() + amt);      // kept for reporting only
    } catch {}
  };
  const push = () => { try { const g = b.graphView(); g.budget = { limit: budget(), spent: spent(), left: left(), lifetime: spentLifetime(), period: 'day' }; onUpdate(g); } catch {} };
  ...
  function recencyPass(opts = {}) {
    try {
      ...
      if (s.dormantAborted) log('[synapse] dormant-project archive HELD BACK — ' + s.dormantAborted);
      // Archiving a project is the most visible thing this pass does, so it is NAMED rather than
      // counted. "47 projects archived" is not something you can check; a list is.
      if (a.dormant) {
        log(`[synapse] archived ${a.dormant} dormant project(s) — begun >${recency.DEFAULTS.dormantStartedDays}d ago, untouched >${recency.DEFAULTS.dormantStaleDays}d:`);
        for (const d of r.plan.dormant.slice(0, 12)) log(`[synapse]   \u00b7 ${d.label} (${d.reason})`);
        if (r.plan.dormant.length > 12) log(`[synapse]   \u00b7 \u2026and ${r.plan.dormant.length - 12} more`);
      }
      if (a.revived) for (const v of r.plan.revive) log(`[synapse] revived ${v.label} \u2014 ${v.reason}`);
      if (a.pruned || a.orphaned || a.refreshed || a.reranked || a.dormant || a.revived) {
  ...
  return {
    ...
    stats: () => b.stats(),
    graphView: () => { const g = b.graphView(); g.budget = { limit: budget(), spent: spent(), left: left(), lifetime: spentLifetime(), period: 'day' }; return g; },
    budget: () => ({ limit: budget(), spent: spent(), left: left(), lifetime: spentLifetime(), period: 'day' }),
