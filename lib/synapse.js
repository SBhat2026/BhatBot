...
function createSynapse({ dir, deps = {} } = {}) {
  ...
  // ── FREE pass ───────────────────────────────────────────────────────────────────────────────────
  async function hydrate({ repos = true, memories = true, notion = true } = {}) {
    const ts = now();
    let added = 0;
  ...
  async function connect({ threshold = 0.8, maxRationale = 8 } = {}) {
    ...
    b.save(); push();
    return { proposed: cands.length - rejected, embedded, rationalized, rejected, ...b.stats() };
  }
  ...
  async function suggest() {
    ...
    let suggestions = [];
    try {
      const txt = String(await llm({ system: SUGGEST_SYSTEM, content: ctx, maxTokens: 700 }) || '').trim();
  ...
  async function tick({ force = false } = {}) {
    ...
    const out = { hydrated: null, connected: null, suggested: null };
    ...
    if (!c.paid) out.paidSkipped = 'paid pass disabled';
      ...
      const what = out.connected.skipped
        ...
        : `${out.connected.proposed} link(s) proposed${out.connected.rejected ? `, ${out.connected.rejected} rejected` : ''}`;
...
module.exports = { createSynapse, scanRepos, readSemanticRecords, SY_PRICE, tok, RATIONALE_SYSTEM, SUGGEST_SYSTEM };
