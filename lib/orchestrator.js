...
async function runRole(role, task, deps, opts = {}) {
  ...
  while (steps++ < maxSteps) {
    ...
    for (const tu of tus) {
      ...
      results.push({ type: 'tool_result', tool_use_id: tu.id, content: JSON.stringify(rClean).slice(0, 16 * 1024), is_error: r && r.success === false });
    }
...
async function fleet(tasks, deps, opts = {}) {
  ...
  const run = (t, i) => {
    ...
    const persona = t.persona || `You are "${role}", ONE suit in BhatBot's agent Vanguard. You work AUTONOMOUSLY on your OWN assigned task, in parallel with other suits. Stay focused on your task only, complete it, and report a concise result. Siddhant may send live feedback mid-task — honor it.`;
...
async function ensemble(task, deps, opts = {}) {
  ...
  const takes = await Promise.all(roles.map((r) => {
    ...
    return runRole(r, task, deps, { ...opts, onUpdate: (p) => { if (opts.onUpdate) try { opts.onUpdate({ id: r.name, role: r.name, ...p }); } catch {} } })
