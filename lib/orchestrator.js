...
// Returns { text, error } — never a bare null. A swallowed failure here was invisible: the caller
// fell back to concatenation, so a BROKEN integrator looked identical to one that had nothing to do.
async function integrateFleet(list, agents, board, deps) {
  try {
    ...
    return text ? { text, error: null } : { text: null, error: 'integrator returned no text' };
...
async function runRole(role, task, deps, opts = {}) {
  ...
  const _traceT0 = Date.now(); const _trace = [];   // procedural learning: this suit's step-series
  while (steps++ < maxSteps) {
    ...
    for (const t of tus) if (_trace.length < 40 && !BOARD_TOOL_NAMES.includes(t.name)) _trace.push(t.name);   // procedural learning (skip board-coordination tools)
  ...
  // Bank this suit's step-series into the shared skill bank (main.js injects deps.recordTrace).
  if (deps.recordTrace && _trace.length) try { deps.recordTrace(String(task), _trace, finalText !== '(stopped)', Date.now() - _traceT0); } catch {}
...
async function fleet(tasks, deps, opts = {}) {
  ...
  let integrated = null, integrateError = null;
  if (opts.integrate !== false && agents.length > 1) {
    ...
    const r = await integrateFleet(list, agents, board, deps);
    integrated = r.text; integrateError = r.error;
    if (integrateError) {
      // Surface it rather than quietly handing back raw per-suit output.
      try { if (board) board.post({ agent: 'LEAD', kind: 'need', text: 'integration failed: ' + integrateError }); } catch {}
      if (deps.log) try { deps.log('[fleet] integration failed: ' + integrateError); } catch {}
    }
    if (opts.onUpdate) try { opts.onUpdate({ id: 'lead', role: 'integrator', status: integrateError ? 'failed' : 'done', text: integrated || integrateError || '' }); } catch {}
  }
  const result = integrated || agents.map((a) => `### ${a.role}\n${a.result}`).join('\n\n');
  return { success: true, mode: 'fleet', ms: Date.now() - t0, agents, result, integrated: !!integrated, integrateError };
}
...
async function ensemble(task, deps, opts = {}) {
  ...
  let synthesis = merged, synthesisError = null, synthesized = false;
  try {
    const resp = await deps.anthropicRequest({ model: deps.models.sonnet, max_tokens: 4096, system: sys,
      messages: [{ role: 'user', content: `Task:\n${task}\n\nThe specialists' takes:\n\n${merged}` }] }, deps.apiKey);
    const text = (resp.content || []).filter((b) => b.type === 'text').map((b) => b.text).join('\n').trim();
    if (text) { synthesis = text; synthesized = true; }
    else synthesisError = 'synthesizer returned no text';
  } catch (e) { synthesisError = String((e && e.message) || e).slice(0, 300); }
  // Falling back to the concatenated takes is the right BEHAVIOUR, but it must not be a silent one:
  // "here are N separate opinions" reads identically whether the lead declined to merge them or
  // simply crashed. Report it so a broken synthesizer is noticed rather than quietly tolerated.
  if (synthesisError) {
    try { if (board) board.post({ agent: 'LEAD', kind: 'need', text: 'synthesis failed: ' + synthesisError }); } catch {}
    if (deps.log) try { deps.log('[ensemble] synthesis failed: ' + synthesisError); } catch {}
  }
  return { success: true, mode: 'ensemble', roles: roles.map((r) => r.name), ms: Date.now() - t0, takes, result: synthesis, synthesized, synthesisError };
}
