...
let ws = null, stopped = false, backoff = 1000, pingTimer = null, authFails = 0;
...
function start({ url, token, executeTool, log = () => {} }) {
  ...
  stopped = false;
  authFails = 0;                                   // a restart is a fresh chance for a corrected token
  ...
  function connect() {
    ...
    ws.on('error', (e) => {
      ...
      // A 401/403 is the cloud REJECTING this token, not a network blip. Reconnecting on a fixed
      // 30s ceiling meant re-presenting a known-bad credential to a public host forever, and
      // logging the failure forever with it. Give up after a few, and say what to fix.
      if (/\b(401|403)\b|Unauthorized|Forbidden/i.test(msg) && ++authFails >= 3) {
        stopped = true;
        log('[cloud-bridge] token rejected by the cloud (' + msg.slice(0, 60) + ') — giving up; set a valid cloudToken and restart');
      } else {
        log('[cloud-bridge] error: ' + msg);
      }
      try { ws.close(); } catch {}
    });
  }
  ...
  function connected() { try { return !!(ws && ws.readyState === 1); } catch { return false; } }
  return { stop() { stopped = true; clearInterval(pingTimer); try { ws && ws.close(); } catch {} }, send, connected };
}
