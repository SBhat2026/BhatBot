...
const CONFIRM_TOOLS = new Set([
  ...
  // make_model executes model-authored Python inside Blender. lib/blender.js allow-lists the imports
  // (at runtime, not just textually) and strips the environment to the same list a third-party MCP
  // plugin gets — but that is a guardrail against carelessness, not a sandbox. So it sits exactly
  // where claude_code sits: autonomous at the local keyboard, a human over a remote channel.
  'make_model',
]);
...
function classifyDesire(desire) {
  ...
  // A file holding a ⚠ LOAD-BEARING invariant is never a routine local edit, however small the
  // change looks. That is the entire point of the marker: these are the places where the obvious
  // simplification is a bug that has already shipped once. Caution, not a block — the work is often
  // legitimate, it just must not be waved through as "localized and verify-testable".
  const marked = markedIn(files);
  if (marked.length)
    return { level: 'STRUCTURAL', decision: 'proceed_caution',
      reason: `touches a load-bearing invariant in ${marked.join(', ')} — read the marker before changing it`, files, marked };
...
// SELF-DEGRADATION firewall (Siddhant's rule, 2026-07-01): once a self-improvement session is
// APPROVED it may run free on whatever it actually affects — EXCEPT it may never degrade itself.
// "Self-degradation" is exactly two things, unified here so the governor expresses the rule directly:
//   1. Editing a file that would let it weaken its own rails (the frozen zone: risk.js/selfdrive/
//      selfheal/security/credentials/admission/verify scripts).
//   2. A plan whose stated intent is to weaken/bypass/disable a guardrail, gate, limit, or check.
// This is the ONE hard block that survives approval; caps/cooldowns are cost controls, not this.
// (The verify-gate + auto-revert in selfdrive.js is the *behavioral* half of the same ban — a change
// that breaks the test suite is also self-degradation and gets reverted.) → { degrading, reason, hits }
function isSelfDegrading(fileList, text = '') {
  const frozen = checkFrozen(fileList);
  const files = (Array.isArray(fileList) ? fileList : String(fileList || '').split(/\s+/)).map((f) => String(f || '').trim());
  const weakensGuardrail = GUARDRAIL_RE.test(String(text || '')) || files.some((f) => f.includes('selfdrive') || f.includes('risk.js'));
  if (frozen.blocked) return { degrading: true, reason: 'would edit a frozen rail: ' + frozen.hits.join(', '), hits: frozen.hits };
  if (weakensGuardrail) return { degrading: true, reason: 'plan intends to weaken/bypass a guardrail', hits: frozen.hits };
  return { degrading: false, hits: [] };
}
...
// The frozen zone above is FILE-level and safety-scoped: eight files selfdrive may never edit. It
// cannot express "this function inside an otherwise editable file is load-bearing, and the obvious
// simplification of it is a bug we have already shipped once".
//
// That case is common here and expensive. A recurring pattern in this codebase is a subtle invariant
// being quietly reverted by a later, reasonable-looking edit:
//   • a test asserted `toolselect.select(lastUserText(history), …)` — pinning a BUG as the contract;
//   • the weaver's EMBED phase returned early whenever work remained, which looked like correct
//     prioritisation and livelocked the loop for ten days;
//   • `hist.slice(-32)` is the obvious way to window a conversation, and it took two sub-agents down.
//
// So: a `⚠ LOAD-BEARING` marker in a comment is a machine-readable claim, not decoration.
// scripts/test-markers.js requires every one to explain WHY (they are useless otherwise), and
// classifyDesire below escalates any desire that targets a marked region, so the autonomous loop has
// to route it past a human instead of tidying it away.
const MARKER_RE = /⚠\s*(LOAD-BEARING|DO NOT (?:TOUCH|CHANGE|EDIT))/i;

/**
 * Find the marked regions of a source file.
 * A marker governs the comment block it appears in plus the declaration immediately after it.
...
function findMarkers(source) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // The justification is the rest of the comment block the marker opens.
    const why = [];
    for (let j = i; j < Math.min(lines.length, i + 40); j++) {
      const t = lines[j].trim();
      if (!/^(\/\/|\*|\/\*)/.test(t)) break;
      why.push(t.replace(/^(\/\/+|\*+|\/\*+)\s?/, ''));
    }
    out.push({ line: i + 1, text: lines[i].trim(), why: why.join(' ').trim() });
  }
  return out;
}
...
/**
 * Which of these files carry a load-bearing marker? Reads from disk, best-effort: an unreadable file
 * is simply not marked, because failing to classify must never crash the governor that calls it.
 */
function markedIn(fileList, { root = path.join(__dirname, '..'), fsMod = fs } = {}) {
  const out = [];
  for (const f of (Array.isArray(fileList) ? fileList : [])) {
    const rel = String(f || '').trim();
    if (!rel) continue;
    for (const cand of [path.resolve(root, rel), path.resolve(rel)]) {
      try {
        if (!fsMod.existsSync(cand) || !fsMod.statSync(cand).isFile()) continue;
        if (findMarkers(fsMod.readFileSync(cand, 'utf8')).length) out.push(rel);
        break;
      } catch { /* unreadable → treat as unmarked */ }
    }
  }
  return [...new Set(out)];
}
...
module.exports = {
  ...
  FROZEN_ZONE, checkFrozen, desireFiles, classifyDesire, severeConcern, isSelfDegrading,
  MARKER_RE, findMarkers, markedIn,
};
