...
// Result keys whose value is a base64 binary payload, not text. See the note in redactDeep.
const BINARY_KEYS = new Set(['_image', 'image', 'shot', 'screenshot', 'thumbnail', 'ulaw', 'audio', 'pdf']);
...
function redactDeep(value, opts = {}) {
  ...
  for (const k of Object.keys(value)) {
    // NEVER walk a base64 payload. Standard base64 contains '+' and '/', which are word boundaries,
    // so a `\bAIza…` or `\bAKIA…` run can match INSIDE an image and the replacement silently
    // corrupts it — verified: 'SGVsbG8+AIzaSyD…' becomes 'SGVsbG8+[REDACTED]+…'. The `\b` anchors
    // protect a continuous base64 run, but not one that happens to follow a + or /.
    //
    // There is nothing to find in there anyway: a secret does not survive base64 encoding in a form
    // any of these rules could match. Skipping is both safer and cheaper (a 400KB screenshot costs
    // ~1.4ms per tool call otherwise). Buffers are already skipped above for the same reason.
    if (BINARY_KEYS.has(k) && typeof value[k] === 'string') continue;
    try { value[k] = redactDeep(value[k], next); } catch { /* frozen/getter-only — leave it */ }
  }
  return value;
}

module.exports = { redact, redactDeep, hasSecret, setSecretProvider, MARK, SHAPES, MIN_VALUE_LEN, BINARY_KEYS };
