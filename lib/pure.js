'use strict';
// Pure, stateless helpers extracted from main.js — the main.js split (see BACKLOG.md "Split main.js"). These depend on nothing
...
// LOOP BREAKER. The canonical autonomous-agent failure (AutoGPT's 400-identical-calls-in-5-minutes)
// is not "runs too long" — it's repeating one call that will never start working. `unproductive`
// above already counts iterations with nothing novel, but it only ever ENDS the run; by then the
// budget is spent. This detects the tighter pattern — the SAME (tool, args) signature landing
// repeatedly in a short window — early enough to redirect the model instead of just cutting it off.
//
// Deliberately keyed on the exact signature: a retry with CHANGED arguments is legitimate
// problem-solving and must not be punished. Pure, so it's unit-testable.
function detectLoop(sigs = [], { window = 8, threshold = 3 } = {}) {
  const recent = sigs.slice(-window);
  const counts = new Map();
  for (const s of recent) counts.set(s, (counts.get(s) || 0) + 1);
  let worst = null, n = 0;
  for (const [sig, c] of counts) if (c > n) { worst = sig; n = c; }
  if (n < threshold) return null;
  const name = String(worst).split(':')[0];
  return { sig: worst, name, count: n, window: recent.length };
}
...
function estimateToolCost(name, input, result) {
  ...
  // Video is the most expensive thing BhatBot can do — roughly two orders of magnitude above an
  // image — and it was costed at ZERO, so the daily ledger under-reported the single tool most worth
  // watching. Priced per second of output, scaled by resolution (AceData bills ~$0.146/s at 720p on
  // the 2.0 line). Approximate by design: the exact rate depends on which model the call selected.
  if (name === 'visual_build') {
    const secs = Math.max(1, Number(input && input.duration) || 5);
    const mult = { '480p': 0.5, '720p': 1, '1080p': 2.2, '4k': 6 }[(input && input.resolution) || '720p'] || 1;
    return +(secs * 0.146 * mult).toFixed(3);
  }
  return 0;
}
...
// Anything that can only be answered by inspecting Siddhant's own machine, data, or agents.
// Deliberately requires a possessive/locative or a concrete artifact word, so a general-knowledge
// question ("who won the world cup") does not get dragged in.
const _INTK_LOCALSTATE = /\b(my|our|this machine|on disk|locally)\b.*\b(files?|folders?|repos?|projects?|inbox|email|mail|calendar|schedule|notes?|memory|memories|jobs?|agents?|tasks?|disk|storage|graph|connections?)\b|\b(files?|commits?|branches?|repos?)\b.*\b(changed|modified|added|deleted|today|this week|yesterday|recently)\b|\bwhat (?:did|have) (?:you|we|i)\b|\b(?:in|under|inside) (?:bhatbot|prism|fable|nexus|mandarin[- ]learn|soccer[- ]game|uricase|protfunc)\b|\bwhat'?s (?:in|on) my\b/i;
...
function classifyIntake(text, opts = {}) {
  ...
  // LOCAL-STATE QUESTIONS are not chat, however question-shaped they look. "What is the capital of
  // France" is answerable from the model; "what files changed in bhatbot today" is answerable only
  // by looking at THIS machine. The question form was sending the second kind down the tool-less
  // path, which produces a confident answer without having looked — the exact failure the intake
  // router exists to prevent. Measured: this was the one broken-task miss in scripts/bench-app.js.
  if (_INTK_LOCALSTATE.test(t)) return 'action';
...
// conversationContinuity(history) — when the CURRENT user turn is a terse, referential follow-up
// ("and the other one", "do that", "what about it?"), name the subject of his PREVIOUS turn so the
// model resolves the reference instead of asking him to repeat. Returns a one-line directive, or ''
// when there's nothing to add (fresh topic, explicit subject, or no history). Pure + testable.
const _CC_REFERENTIAL = /^(and|also|what about|how about|that one|the other|the second|the first|same|again|do (it|that|this)|go on|continue|keep going|more|why|why not|really\??)\b/i;
const _CC_PRONOUN_ONLY = /\b(it|that|this|those|these|them|they|one)\b/i;
function _ccText(m) {
  if (!m) return '';
...
const _CC_STOPWORD = /^(I|The|A|An|It|That|This|These|Those|You|We|My|Your|Show|Tell|Explain|Give|Let|Please|Can|Could|Would|Should|Do|Does|Open|Render|Make|Find|Get|Set|What|Why|How|When|Where|Who|Which|Is|Are|Was|Were)$/;
function _ccSubject(text) {
  const t = String(text || '').replace(/\s+/g, ' ').trim();
  if (!t) return '';
  const quoted = t.match(/["'`]([^"'`]{2,60})["'`]/);
  if (quoted) return quoted[1];
  // Runs of Capitalized words (proper-noun phrases), minus leading command/question words.
  const props = (t.match(/\b([A-Z][\w-]+(?:\s+[A-Z][\w-]+){0,3})\b/g) || []).filter((p) => !_CC_STOPWORD.test(p));
  // Prefer an ALL-CAPS acronym (FABLE / SYNAPSE / PRISM) — the strongest subject signal — else the longest phrase.
  const acro = props.filter((p) => /^[A-Z0-9]{2,}$/.test(p)).sort((a, b) => b.length - a.length)[0];
  if (acro) return acro;
  const best = props.sort((a, b) => b.length - a.length)[0];
  if (best) return best;
  const words = t.split(' ').filter(Boolean);
  return words.slice(0, 6).join(' ');
}
function conversationContinuity(history) {
  if (!Array.isArray(history) || history.length < 2) return '';
  const users = history.filter((m) => m && m.role === 'user');
  if (users.length < 2) return '';
  const cur = _ccText(users[users.length - 1]).trim();
  const wc = (cur.match(/[a-z0-9']+/gi) || []).length;
  const referential = _CC_REFERENTIAL.test(cur) || (wc <= 6 && _CC_PRONOUN_ONLY.test(cur));
  if (!referential) return '';
  // Prefer the subject Siddhant himself raised in his previous turn, then what BhatBot last discussed.
  const lastAsst = [...history].reverse().find((m) => m && m.role === 'assistant');
  const subject = _ccSubject(_ccText(users[users.length - 2])) || _ccSubject(_ccText(lastAsst));
  if (!subject || subject.length < 2) return '';
  return `CONTINUITY: Siddhant's message is a short follow-up to the previous exchange about "${subject}". Resolve any "it/that/the other/again" against that subject — do not ask him to restate what he means.`;
}

// shouldRaiseFleet(payload, others, msSinceLastShown) — should the app bring the FLEET office up?
//
// Three tools used to raise the panel themselves, so the auto-switch existed exactly three times and
// covered nothing else: a DAG fan-out, a delegated project or a background specialist all started
// work with the panel still showing chat. The decision now lives at the single point every agent
// heartbeat passes through (main.js fleetBroadcast) — and here, pure, because "when does the screen
// move on its own" is a rule worth being able to read and test rather than infer from a call site.
//
// A screen that yanks itself around is worse than one that never moves, so it is gated three ways:
//   • only on a genuinely active state — an agent FINISHING is not a reason to switch
//   • only when nothing else was already working, so a five-agent fan-out switches once, not five times
//   • never twice within a minute
//   • never for the idle backlog worker, which runs while Siddhant is away and is not his request
//
// @param payload  the agent heartbeat: { id, state|status, background? }
// @param others   every OTHER agent currently known: [{ id, state|status }]
// @returns {{ raise: boolean, why: string }} — `why` is the reason either way, so a switch that did
//          not happen is explainable instead of merely absent.
const _FLEET_ACTIVE = /^(working|thinking|start)/;
const _FLEET_BUSY = /^(working|thinking)/;
const FLEET_RAISE_COOLDOWN_MS = 60000;
function shouldRaiseFleet(payload = {}, others = [], msSinceLastShown = Infinity) {
  if (payload.background) return { raise: false, why: 'background worker — not something Siddhant asked for' };
  const st = String(payload.state || payload.status || '');
  if (!_FLEET_ACTIVE.test(st)) return { raise: false, why: `state "${st || 'none'}" is not the start of work` };
  const busy = (others || []).filter((a) => a && a.id !== payload.id && _FLEET_BUSY.test(String(a.state || a.status || '')));
  if (busy.length) return { raise: false, why: `${busy.length} agent(s) already working — the office is already up` };
  if (msSinceLastShown < FLEET_RAISE_COOLDOWN_MS) return { raise: false, why: 'raised within the last minute' };
  return { raise: true, why: 'first agent to start work in a quiet office' };
}

module.exports = { detectLoop, textHintFromSelector, splitForSpeech, estimateToolCost, stripReasoning, classifySpeech, createSpeechNormalizer, isPromissory, shouldExtendBudget, toolSig, progressLine, looksActionable, classifyIntake, conversationContinuity, shouldRaiseFleet, FLEET_RAISE_COOLDOWN_MS };
