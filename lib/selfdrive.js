...
const DEFAULTS = {
  ...
  maxReviewRevisions: 2,     // REVIEWER stage: re-FORGE this many times on a severe finding, then block
  minImpact: 'low',
...
async function runCycle(loadConfig, deps, session) {
  ...
  // 5b. REVIEWER — a skeptic pass over the DIFF against CODE_STYLE.md + the frozen rules, BEFORE the
  //     verify judge. Optional (deps.review absent → skipped, behavior unchanged). A `severe` finding
  //     triggers a bounded revise loop (re-FORGE with the reviewer's notes); if still severe after
  //     maxReviewRevisions, revert + blocked_attempt so bad-quality code never ships even if it'd pass
  //     tests. This is the demo-7 quality gate.
  if (deps.review && forgeRes && forgeRes.success !== false) {
    const maxRev = cfg.maxReviewRevisions != null ? cfg.maxReviewRevisions : 2;
    let round = 0;
    while (true) {
      let diff = '';
      try { const d = await runShell('git diff', proj); diff = String((d && d.stdout) || '').slice(0, 20000); } catch {}
      let rv = null;
      try { rv = await deps.review({ diff, desire, plan: planOut, style: cfg.styleDoc }); } catch { rv = null; }
      emit('REVIEWER', rv ? 'severity: ' + (rv.severity || 'none') : 'review unavailable', { round });
      if (!rv || String(rv.severity).toLowerCase() !== 'severe') break;   // none/minor → proceed to verify
      if (round++ >= maxRev) {
        await runShell('git checkout -- . && git clean -fd', proj);
        const n = markAttempt(desire.id);
        const detail = String(rv.notes || 'severe review findings').slice(0, 180);
        if (resolveDesire) resolveDesire(desire.id, { summary: `reviewer blocked (severe, ${maxRev} revisions tried): ${detail}`, helped: false });
        if (notify) notify(`↩ self-drive: “${desire.id}” reviewer blocked — severe quality/style issues after ${maxRev} revisions. Reverted, nothing shipped.`);
        emit('REVIEWER', 'blocked — severe after revisions', { detail });
        return { outcome: n >= cfg.maxAttemptsPerDesire ? 'human_review_needed' : 'blocked_attempt', desire: desire.id, reason: 'reviewer_severe', attempts: n, degradation: { kind: 'reviewer_severe', detail } };
      }
      emit('FORGE', 'revising per reviewer', { round });
      try { forgeRes = await forge({ desire, classification, report, plan: { ...planOut, reviewerNotes: rv.notes }, verify: verifyCmd, proj }); }
      catch (e) { forgeRes = { success: false, error: e.message }; break; }
    }
  }
