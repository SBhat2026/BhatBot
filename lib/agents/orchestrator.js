...
async function run(goal, { wsDir, config = {}, adapters = {}, maxTasks = 30, checkpointEvery = 5, concurrency = 3, onStep, onTask, shouldStop, getGuidance } = {}) {
  ...
  while (done < maxTasks) {
    ...
    // Cross-agent relay: hand each task the most recent sibling findings so agents build on each
    // other instead of working blind in parallel. Bounded (last 6) to keep context flat.
    const peers = working.slice(-6);
