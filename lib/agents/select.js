'use strict';
// DAG agent model/provider SELECTOR (Phase 2, 2026-06-26 — relocated from the former lib/router.js).
//
// This is NOT the main chat router. The single live CHAT routing path is main.js: chooseModel
// (regex + telemetry nudge) + the callModel preflight (per-model ITPM/OTPM budgeting + OpenAI/
// Gemini/Ollama text-offload rungs). That path was unified in Phase 1.
//
// This module is the SEPARATE provider/model selector for the stateless DAG role agents
// (lib/agents/base.js, lib/agents/orchestrator.js). Those agents run with INJECTED adapters
// (provider-agnostic) and their own per-class escalation chains — they are not the chat loop and
// cannot share chooseModel/callModel (which are Electron-internal: loadConfig, darkbloom, rate
// windows, buildSystemPrompt). Phase 2 retired the top-level lib/router.js and moved this genuinely
// load-bearing logic into the agents package where its only consumers live. The offline harnesses
// (scripts/perf-eval.js, scripts/test-pass39.js) import the deterministic bits (classOf / pick /
// shouldEscalate) from HERE now. See ARCHITECTURE.md.
//
// Picks the cheapest model that can plausibly do the task, escalates on failure/complexity, falls
// back when a provider is down, and enforces the monthly $ cap (cost governor).
...
// `schema` is threaded through to the anthropic adapter so a caller can have its JSON shape ENFORCED
// (output_config.format) instead of parsed out of prose. Only the anthropic rung honours it — the
// local/offload rungs have no equivalent, which is exactly why callers must keep a parse fallback.
async function run(choice, { messages, system, task, schema, schemaName }, adapters = {}) {
  switch (choice.provider) {
    case 'ollama': return { text: await adapters.ollamaChat(messages, system, choice.model), model: choice.model, usd: 0 };
    case 'anthropic': return { text: await adapters.anthropic(messages, system, choice.model, { schema, schemaName }), model: choice.model, usd: null /* caller records via recordCost */ };
...
// Pricing comes from lib/pricing.js — the SAME table main.js bills against. This module used to
// keep its own copy, and the two drifted: Opus sat at the retired 4.1 rate of $15/$75 here while
// billing $5/$25. That matters more here than anywhere else, because these numbers enforce the
// monthly cost CAP below — an overstated price does not merely misreport, it starves the fan-out.
const pricing = require('../pricing');
const PRICES = new Proxy({}, {           // kept for back-compat with callers/tests that read it
  get: (_, k) => { if (typeof k !== 'string') return undefined; const b = pricing.base(k); return b.input || b.output ? [b.input, b.output] : undefined; },
  has: (_, k) => typeof k === 'string' && !!(pricing.base(k).input || pricing.base(k).output),
  ownKeys: () => pricing.known(),
  getOwnPropertyDescriptor: () => ({ enumerable: true, configurable: true }),
});
function estimateUsd(model, inTok, outTok) {
  return +pricing.usd(model, { input_tokens: inTok, output_tokens: outTok }).toFixed(5);
}

module.exports = { DEFAULT_MODELS, CHAINS, PRICES, pick, run, shouldEscalate, scoreComplexity, classOf, estimateUsd };
