...
async function runAgent(task, { wsDir, config = {}, adapters = {} } = {}) {
  ...
  const userMsg = JSON.stringify({ goal: task.goal, state: assembled.state, memory: assembled.memory, files: assembled.files, constraints: assembled.constraints, peer_findings: assembled.peers, expects: task.expects });
