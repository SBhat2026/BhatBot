...
async function runAgent(task, { wsDir, config = {}, adapters = {} } = {}) {
  ...
  while (attempt <= (config.local_retry_limit ?? 2)) {
    ...
    // ---- Plain text path (no-tool roles, or provider lacks tool support) ----
    if (choice.provider === 'anthropic' || choice.provider === 'ollama') {
      ...
      result.cost = result.cost || {}; result.cost.model = choice.model;
      if (choice.provider === 'anthropic' && out.usd === null) result.cost.usd = router.estimateUsd(choice.model, est(userMsg), (result.summary || '').length);
    } else {
