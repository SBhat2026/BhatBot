...
module.exports = function makeMaps(ctx = {}) {
  ...
  // Vector Map ID → enables Google WebGL vector rendering = real 3D buildings + smooth tilt/rotate.
  // Defaults to Google's public DEMO_MAP_ID (works without Cloud Console setup); override with a
  // custom vector Map ID via config.maps.mapId for your own styling.
  const getMapId = ctx.getMapId || (() => '');
  ...
  // Route through an ORDERED list of [lat,lon] points (origin, optional stops, destination) — the
  // interactive in-window planner uses this for click-to-add waypoints and marker dragging (no
  // re-geocoding needed). OSRM (free) supports multi-coordinate routes in one request.
  async function routePath(points, mode) {
    ...
    const j = await jget(`${OSRM}/route/v1/${profile}/${coords}?overview=full&geometries=geojson`);
    if (j.code !== 'Ok' || !j.routes.length) throw new Error(`OSRM: ${j.code || 'no route'}`);
    const rt = j.routes[0];
    return {
      distance_km: +(rt.distance / 1000).toFixed(1),
  ...
  async function prepare(input = {}) {
    const key = getKey();
    const mapId = key ? (getMapId() || 'DEMO_MAP_ID') : undefined;   // vector 3D only when keyed
    ...
    if (action === 'route' || action === 'directions') {
      ...
      return { kind: 'route', googleKey: key || undefined, mapId, ...r };
    }
    const g = await geocode(input.query || input.location || input.place);
    return { kind: 'point', googleKey: key || undefined, mapId, center: [g.lat, g.lon], label: g.label, source: g.source, zoom: input.zoom || 14 };
  }

  return { geocode, route, routePath, prepare };
};
