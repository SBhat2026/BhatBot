...
function classifyDepth(text, cal) {
  ...
  // `cal` (optional) = learned per-tier max_tokens map built from the response dataset
  // (main.js depthCal): { ack, conversational, detailed, deep }. When present for the chosen tier
  // it REPLACES the static ceiling so the budget tracks how much output turns of this kind have
  // actually needed — over-allocation shrinks, chronic clipping grows. The DIRECTIVE stays static
  // (it nudges style, not size).
  const base = TIERS[depth];
  const learned = cal && cal[depth];
  const maxTokens = (learned && learned > 0) ? learned : base.maxTokens;
  return { depth, maxTokens, directive: base.directive };
}
