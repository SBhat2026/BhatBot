...
// SOURCE fix for interruption-orphans (T2): when a turn ENDS (finish / "⏹ Stopped." / barge-in abort)
// with the last assistant message still holding un-answered tool_use blocks — because the abort landed
// between the tool_use emission and its tool_result — append synthetic '[interrupted]' results so the
// pair is intact the moment the history is stored. This runs at the single turn-exit, so no downstream
// wire path ever sees a trailing dangling tool_use (capTokens+validateHistory remain the safety net,
// but the orphan is now prevented at the source). Returns a copy; input untouched.
function sealDanglingToolUse(history) {
  if (!Array.isArray(history) || !history.length) return Array.isArray(history) ? history.slice() : [];
  const out = history.slice();
  const last = out[out.length - 1];
  const blocks = (m) => (Array.isArray(m.content) ? m.content : []);
  if (last && last.role === 'assistant') {
    const ids = blocks(last).filter((b) => b.type === 'tool_use').map((b) => b.id);
    if (ids.length) {
      out.push({ role: 'user', content: ids.map((id) => ({ type: 'tool_result', tool_use_id: id, content: '[interrupted — no result captured]', is_error: true })) });
    }
  }
  return out;
}
...
module.exports = { validateHistory, sealDanglingToolUse, evictOldImages, isRetryableTool, TRANSIENT_RE };
