...
function normalize(history) {
  ...
  for (const r of (history || [])) {
    if (!r) continue;
    const row = { ...r };
    row.sleep_hours = (typeof r.sleep_seconds === 'number') ? round(r.sleep_seconds / SECS_PER_HOUR) : null;
...
module.exports = { METRICS, normalize, trends, flags, portrait, brief, insights, INSIGHT_SYSTEM };
