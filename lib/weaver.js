...
function createWeaver({ synapse, brain, governor = null, isIdle = () => true, now = () => Date.now(), log = () => {}, onEvent = () => {}, config = {} } = {}) {
  ...
  let embedStalled = false;      // embedding has work but cannot do it — do not block the free phases
  let phase = 'idle';
  ...
  function blockedReason() {
    ...
    // ⚠ LOAD-BEARING — A SPENT BUDGET MUST NOT BLOCK THE TICK. It used to return 'budget' here, skipping the
    // whole cycle — including LINK, which is pure local cosine and costs nothing. Two of the four
    // phases need no money at all, and the graph has ~2,000 vectors already paid for; refusing to
    // use them because a future paid phase is unaffordable is throwing away work already bought.
    // The paid phases guard themselves (embedSome, explainEdges and suggest each check `left()`),
    // so the money is still bounded — it is bounded at the point of spending, which is the correct
    // place, instead of at the point of thinking.
    return null;
  }

  /** Reported by status() so the UI can distinguish "out of money" from "nothing to do". */
  function budgetSpent() {
    try { return !!(synapse.budget && synapse.budget().left <= 0); } catch { return false; }
  }
  ...
  async function tick() {
    ...
    try {
      ...
      // ⚠ LOAD-BEARING — a phase that CANNOT MAKE PROGRESS must not hold the pipeline.
      //
      // This used to `return` whenever anything looked unembedded, regardless of whether embedding
      // had actually achieved anything. embedSome() gives up quietly on an exhausted budget, a
      // missing key, or an offline embedder, returning 0 — and the queue does not shrink, so the next
      // tick makes the identical decision. The loop then never reaches LINK, EXPLAIN or SUGGEST
      // again, for the rest of the process's life.
      //
      // That was the live state: 1,821 nodes carried vectors from a retired 1536-d embedder, the $1
      // lifetime budget was spent, so embedSome returned 0 forever and the second brain had produced
      // nothing since 22 August while reporting itself as "on" and busy "embedding". Linking is FREE
      // and local — there is no reason for it to be blocked behind a paid phase that is stuck.
      ...
      if (need.length) {
        ...
        stats.embedded += n;
        if (n > 0) {
          // Progress clears the stall immediately — waiting for the queue to EMPTY would leave the
          // flag stuck on for the whole migration, which is the state it is meant to distinguish from.
          if (embedStalled) { embedStalled = false; log('[weaver] embedding resumed'); }
          return { phase, embedded: n, remaining: need.length - n };
        }
        // Zero embedded with work outstanding = this phase is blocked, not finished. Say so once
        // (a loop that silently does nothing is the failure this whole module exists to prevent),
        // then fall through to the phases that can still run on the vectors we already have.
        if (!embedStalled) {
          embedStalled = true;
          log(`[weaver] embedding is stalled with ${need.length} node(s) outstanding — continuing with linking, which is free`);
          emit('phase', { phase: 'embed-stalled', remaining: need.length });
        }
      } else if (embedStalled) {
        embedStalled = false;
        log('[weaver] embedding resumed');
      }
  ...
  function status() {
    ...
    return { enabled, running, phase, cursor, stats: { ...stats }, budget, blocked: blockedReason(), embedStalled, budgetSpent: budgetSpent(), newLinksSinceSuggest, feed: feed.slice(-20) };
  }
