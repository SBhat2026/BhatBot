...
// gmailAccounts["a@x.com"].appPassword) AND through arrays.
//
// Arrays used to be skipped at both the guard and the recursion step, which left the write-time
// validator with a hole exactly where it mattered most: `mcpPlugins` is an ARRAY of plugin specs,
// and a plugin spec is the one config shape that carries a bearer token
// (mcpPlugins[0].auth.token). findPlaintext returned nothing for it, so sanitizeWrite's
// "no plaintext credentials on disk" check passed it through untouched and the token was written
// to config.json in the clear — silently, with no error and no vault entry. Object.entries() gives
// string indices for arrays and `container[key]` assigns back correctly, so recursing is all it takes.
function findPlaintext(obj, prefix = '', depth = 0, parentObj = null) {
  const hits = [];
  if (!obj || typeof obj !== 'object' || depth > 6) return hits;
  for (const [k, v] of Object.entries(obj)) {
    // An array index is never a secret NAME — only its contents can hold one.
    if (!Array.isArray(obj) && isPlaintextSecret(k, v)) hits.push({ path: prefix + k, key: k, value: v, container: obj });
    else if (v && typeof v === 'object') {
      const p = Array.isArray(obj) ? `${prefix.replace(/\.$/, '')}[${k}].` : prefix + k + '.';
      hits.push(...findPlaintext(v, p, depth + 1, obj));
    }
  }
  return hits;
}

// A CRED_REF handle is `CRED_REF_<LABEL>_<suffix>` and must match /^CRED_REF_[A-Z0-9_]+$/, so a
// label carrying `.` or `[0]` would mint a handle that looksLikeRef() rejects — i.e. a secret that
// vaults and then can never be resolved. Normalizing HERE rather than trusting each store to do it
// keeps that coupling out of the callers. Top-level keys are already safe and pass through unchanged.
function vaultLabel(pathStr) { return String(pathStr).replace(/\W+/g, '_').replace(/^_+|_+$/g, ''); }
...
function sanitizeWrite(patch, { store } = {}) {
  ...
  // Label by PATH, not by bare key: two plugins in mcpPlugins[] both have an `auth.token`, and a
  // shared "token" label would make their vault entries indistinguishable. At the top level the
  // label is unchanged (`apiKey` stays `apiKey`), so existing handles and reconcileVaultRefs() are
  // unaffected.
  for (const h of findPlaintext(next)) h.container[h.key] = store(vaultLabel(h.path), h.value);   // container points into the clone
  return next;
}
...
function migrate(cfg, { store }) {
  ...
  for (const h of findPlaintext(next)) {
    try { h.container[h.key] = store(vaultLabel(h.path), h.value); migrated.push(h.path); }
    catch { /* leave this one; surfaced by caller via the leftover check */ }
  }
  return { next, migrated };
}

module.exports = { isSecretKey, looksLikeRef, isPlaintextSecret, findPlaintext, sanitizeWrite, migrate, vaultLabel, SECRET_KEY_RE };
