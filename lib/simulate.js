...
const REGISTRY = {
  ...
  ase: 'atomistic simulation environment / materials', mujoco: '3D rigid-body / contact / robotics physics (DeepMind; fast, high-fidelity)',
  openmm: 'molecular dynamics', pyscf: 'ab-initio quantum chemistry (HF/DFT/MP2)',
  smolagents: 'code-first agent for complex multi-step math reasoning (used by math_reason)',
};
...
// Complex math reasoning via a smolagents CodeAgent (writes + runs Python with the scientific
// stack authorized). Returns a computed, verifiable answer rather than a guessed one.
function mathReason({ task, model, maxSteps, apiKey, timeoutMs } = {}) {
  if (!isInstalled()) return Promise.resolve({ success: false, error: 'Simulation venv not set up. Run: bash scripts/sim-setup.sh' });
  if (!task || !String(task).trim()) return Promise.resolve({ success: false, error: 'provide a `task` (the problem to reason about)' });
  if (!apiKey) return Promise.resolve({ success: false, error: 'no API key for the reasoning model' });
  const worker = path.join(__dirname, '..', 'scripts', 'smol_agent.py');
  const req = JSON.stringify({ task: String(task), model: model || 'anthropic/claude-sonnet-4-6', api_key: apiKey, max_steps: Math.min(Math.max(Number(maxSteps) || 6, 1), 12) });
  const to = Math.min(Math.max(Number(timeoutMs) || 180000, 1000), 600000);
  return new Promise((resolve) => {
    const p = spawn(pythonBin(), [worker], { timeout: to });
    let out = '', err = '';
    p.stdout.on('data', (d) => out += d); p.stderr.on('data', (d) => err += d);
    p.on('error', (e) => resolve({ success: false, error: 'math_reason spawn failed: ' + e.message }));
    p.on('close', () => {
      let j = null; try { j = JSON.parse(out.trim().split('\n').pop()); } catch {}
      if (!j) return resolve({ success: false, error: (err || 'no result from reasoning agent').slice(0, 1200) });
      if (!j.ok) return resolve({ success: false, error: j.error || 'reasoning failed', trace: j.trace });
      resolve({ success: true, answer: j.answer, steps: j.steps, code: j.code });
    });
    try { p.stdin.write(req); p.stdin.end(); } catch (e) { resolve({ success: false, error: e.message }); }
  });
}

module.exports = { capabilities, run, mathReason, isInstalled, REGISTRY, SIM_VENV, OUT_DIR };
