...
function structure(repoDir, gaps) {
  ...
  // Open debt now comes from BACKLOG.md — ONE list, verified against the code.
  // This used to scan PHASE1..4_NOTES.md, which were build journals from June: most of what they
  // still listed as open had shipped long ago (durable jobs, the agent lock, the triage lane,
  // lib/pure.js, lib/audit.js). So self-introspection kept reporting debt that was already paid,
  // and every consumer of that number was wrong in the same direction.
  const debt = [];
  for (const f of ['BACKLOG.md']) {
    ...
    for (const line of txt.split('\n')) {
      ...
      // ⬜ is BACKLOG.md's open marker and the reason this returned nothing after the consolidation:
      // the old pattern was tuned to the prose of the phase notes ("out-of-scope", "still phantom"),
      // not to a checklist. An extractor that silently matches zero lines reads exactly like "no
      // debt", which is the most misleading answer it could give.
      if (/^[-*]?\s*⬜/.test(t) || (/⚠|out-of-scope|not fixed|TODO|staged|paused|low priority/i.test(line) && t.length > 12)) {
        debt.push({ file: f, note: t.replace(/^[-*>\s⬜]+/, '').slice(0, 180) });
      }
    }
  }
...
function buildSelfPortrait({ toolNames = [], roleNames = [], repoDir = process.cwd() } = {}) {
  const gaps = [];
  let perf, caps, know, struct, hist, code;
  ...
  // code_scan: concrete, file-level improvement opportunities read from the actual source, so
  // reflection can propose desires that NAME a real code surface (required for them to be automatable)
  // — this is what lets self-drive find work without a human-supplied focus.
  try { code = require('./codescan').scan(repoDir); } catch (e) { code = { error: e.message }; }
