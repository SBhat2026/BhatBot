...
const CLI_TOOLS = [
  { name: 'read_file', description: 'Read a UTF-8 text file (100KB max). Absolute paths.', input_schema: { type: 'object', properties: { path: { type: 'string' } }, required: ['path'] } },
...
function cliToolExec(name, input) {
  return new Promise((resolve) => {
    try {
      if (name === 'read_file') return resolve({ success: true, content: fs.readFileSync(input.path, 'utf8').slice(0, 100 * 1024) });
